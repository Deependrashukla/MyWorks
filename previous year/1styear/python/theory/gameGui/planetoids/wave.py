...
class Wave(object):
    ...
    def break_asteroid(self,other,asteroid):
        if other._velocity.x == 0 and other._velocity.y == 0:
            ...
            v2 = Vector2(other._velocity.x * math.cos(degToRad(240)) - other._velocity.y * math.sin(degToRad(240)), other._velocity.x * math.sin(degToRad(240)) + other._velocity.y * math.cos(degToRad(240))).normalize()

        if asteroid._size == LARGE_ASTEROID :
            self._asteroids.append(Asteroid(x = asteroid.x + (v0 * MEDIUM_RADIUS).x, y = asteroid.y + (v0 * MEDIUM_RADIUS).y, size = "medium", velocity=v0 * MEDIUM_ASTEROID, height = 2 * MEDIUM_RADIUS, width = 2 * MEDIUM_RADIUS, source = MEDIUM_IMAGE))
            self._asteroids.append(Asteroid(x = asteroid.x + (v1 * MEDIUM_RADIUS).x, y = asteroid.y + (v1 * MEDIUM_RADIUS).y, size = "medium", velocity=v0 * MEDIUM_ASTEROID, height = 2 * MEDIUM_RADIUS, width = 2 * MEDIUM_RADIUS, source = MEDIUM_IMAGE))
            self._asteroids.append(Asteroid(x = asteroid.x + (v2 * MEDIUM_RADIUS).x, y = asteroid.y + (v2 * MEDIUM_RADIUS).y, size = "medium", velocity=v0 * MEDIUM_ASTEROID, height = 2 * MEDIUM_RADIUS, width = 2 * MEDIUM_RADIUS, source = MEDIUM_IMAGE))


        elif asteroid._size == MEDIUM_ASTEROID :
            self._asteroids.append(Asteroid(x = asteroid.x + (v0 * SMALL_RADIUS).x, y = asteroid.y + (v0 * SMALL_RADIUS).y, size = "small", velocity=v0 * MEDIUM_ASTEROID, height = 2 * SMALL_RADIUS, width = 2 * SMALL_RADIUS, source = SMALL_IMAGE))
            self._asteroids.append(Asteroid(x = asteroid.x + (v1 * SMALL_RADIUS).x, y = asteroid.y + (v1 * SMALL_RADIUS).y, size = "small", velocity=v0 * MEDIUM_ASTEROID, height = 2 * SMALL_RADIUS, width = 2 * SMALL_RADIUS, source = SMALL_IMAGE))
            self._asteroids.append(Asteroid(x = asteroid.x + (v2 * SMALL_RADIUS).x, y = asteroid.y + (v2 * SMALL_RADIUS).y, size = "small", velocity=v0 * MEDIUM_ASTEROID, height = 2 * SMALL_RADIUS, width = 2 * SMALL_RADIUS, source = SMALL_IMAGE))