...
class Bullet(GEllipse):
    ...
    def __init__(self,velocity,x, y, width=2*BULLET_RADIUS, height=2*BULLET_RADIUS, fillcolor=BULLET_COLOR):
...
class Ship(GImage):
    ...
    def __init__(self,x,y,angle,width,height,source):
    ...
    def impulse(self,input):
        ...
        if input.is_key_down('up'):
            ...
            if self._velocity.length()>SHIP_MAX_SPEED:
                self._velocity=SHIP_MAX_SPEED*self._velocity.normalize()
...
class Asteroid(GImage):
    ...
    def __init__(self,x,y,width,height,source,size,velocity):
