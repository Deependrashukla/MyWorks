...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        self._title = GLabel(text="planetoids", font_size = TITLE_SIZE, font_name = TITLE_FONT, x = GAME_WIDTH/2, y=GAME_HEIGHT/2+ TITLE_OFFSET)
        self._message=GLabel(text="press 's' to start", font_size= MESSAGE_SIZE,font_name = MESSAGE_FONT, x = GAME_WIDTH/2, y=GAME_HEIGHT/2+ MESSAGE_OFFSET)
    ...
    def update(self,dt):
        ...
        # IMPLEMENT ME
        if self.input.is_key_down('s') or self.input.is_key_down('S'):
            print('a')
        ...
        # if self._wave != None and self._wave._ship==None:
        #     print('c')
        #     self._state = STATE_PAUSED

        # print('e')
        # if self._wave != None and self._wave._lives<=2:
        #     print('d')
        #     self._state=STATE_INACTIVE
    ...
    def draw(self):
        ...
        # if self._title==None and self._message==None:
            # return 
        GLabel(text="", font_size = TITLE_SIZE, font_name = TITLE_FONT,width=GAME_WIDTH*2,height=2*GAME_HEIGHT,fillcolor=RGB(200,11,255)).draw(self.view)
        if self._state==STATE_INACTIVE:
