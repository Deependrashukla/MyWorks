...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        self._sdown = False
        self._wave  = None
        self._title = self._make_label('PLANETOIDS', "t")
        self._message = self._make_label('Press "S" to Play',"m")
    ...
    def update(self,dt):
        ...
        # self._determineStates()     # check new state and set it to _state
        # print(self._state)
        ...
        #     self._wave = Wave(data)               # making a wave

        #     # start the game by loading new state
        #     self._state = STATE_ACTIVE

        # elif self._state == STATE_ACTIVE:
        #     self._wave.update(self.input)
        #     if self._wave.is_wave_ended():
        ...
        #     self._wave.update(self.input)
            ...
            self._message = GLabel(text = "Thankyou! Game Over", font_size = MESSAGE_SIZE, left = 150,
                        font_name = MESSAGE_FONT, linecolor = 'black',  bottom = 300)
    ...
    def _make_label(self,msg,type):
        """
        Adds a new message, centered in the screen.

        Parameter msg: The message to display
        Precondition: msg is a string

        Parameter size: The font size
        Precondition: size is a number (int or float)

        Parameter font: The font to display the message in
        Precondition: font is the name of a file in folder Fonts
        """    
        ...
        if type=="t":
            ...
            color = "red"
            x=GAME_WIDTH/2.0
            y=GAME_HEIGHT/2.0 + TITLE_OFFSET
        else:
            font = MESSAGE_FONT
            size = MESSAGE_SIZE
            color = "black"
            x=GAME_WIDTH/2.0
            y=GAME_HEIGHT/2.0 + MESSAGE_OFFSET

        message = GLabel(x=x,y=y,text=msg)
        message.font_size = size
        message.linecolor = color
        message.bold = True
    ...
    def _determineState(self):
        """
        Computes the current state of the application
        """
        spress = self.input.is_key_down('s') and not self._sdown

        if self._state == STATE_INACTIVE and spress:
            self._state = STATE_LOADING
            self._title = None
            self._message = None
        elif self._state == STATE_PAUSED and spress:
            self._state = STATE_CONTINUE
        #elif self._state == STATE_ACTIVE and self._wave.hasWon():
        #    self._state = STATE_COMPLETE
        #    self._message = self._make_label('Wave Complete',60)
        #elif self._state == STATE_ACTIVE and self._wave.hasLost():
        #    self._state = STATE_COMPLETE
        #    self._message = self._make_label('Game Over',60)
        elif self._state == STATE_ACTIVE and not self._wave.hasShip():
            self._state = STATE_PAUSED
            self._message = self._make_label("Press 'S' to Continue",60)
