...
class Encoder(a6filter.Filter):
    ...
    def decode(self):
        ...
        current = self.getCurrent()
        blist = []
        lst = [self._decode_pixel(0), self._decode_pixel(1), self._decode_pixel(2)]
        text = bytes(lst).decode('utf-8')
        ...
        if text == 'Tru':
            ...
            for i in range(10, int(limit_text_loop) + 10):
                ...
                if not (self._decode_pixel(i) > 255):
    ...
    def _encode_pixel(self, pos, text):
        ...
        current.setPixel(pos // current.getWidth(), pos % current.getHeight(), pixel)
