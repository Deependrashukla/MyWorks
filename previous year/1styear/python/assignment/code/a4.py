...
def draw_triangle(t, s, c):
    ...
    old_color = t.color  # Here we storing the color.
    t.color = c
    old_drawmode = t.drawmode
    t.drawmode = True
    ...
    t.color = old_color  # Here we setting turtle back to old color as per precondition.
    t.drawmode = old_drawmode
    t.flush() # For speed 0 case.
...
def draw_spiral(w, side, ang, n, sp):
    ...
    w.clear()                        # Here we clear the window.
    t = Turtle(w)                    # Here we created a new turtle of window w.
    t.left(90)
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    for i in range(1, n + 1):
        t.color = color_lst[i % 3]
        t.forward(side * i)
        t.left(ang)

    # Here we are assigning old values to attributes.
    t.speed = old_speed
    t.color = old_color
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_valid_iteration(k), report_error('Invalid iteration number', k)
    assert type(n) == int and n >= 3, report_error('Invalid number of side', n)
...
def radiate_petals(w, radius, width, n, sp):
    ...
    assert type(n) == int and n >= 2, report_error('Invalid number of lines', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def vicsek(w, size, d, sp):
    ...
    assert is_valid_length(d), report_error('Invalid depth', d)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def vicsek_helper(p, x, y, size, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('Invalid number', x)
    assert is_number(y), report_error('Invalid number', y)
    assert is_valid_length(size), report_error('Invalid size', size)
    assert is_valid_length(d), report_error('Invalid depth', d)
...
def branches(w, hght, ang, d, sp):
    ...
    assert is_number(ang), report_error('Invalid angle it should be in between 0 and 180', ang)
    assert is_valid_length(d), report_error('it is not a valid depth', d)
    assert is_valid_speed(sp), report_error('It is not valid speed', sp)
